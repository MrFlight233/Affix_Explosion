...
> 编号：**07** · [文档地图](./README.md)  
> 选定方向：A · 渐进优化 + 工作风隐身模式  
> 编译日期：2026-07-20  
> 相关文档：[01-游戏概述](./01-游戏概述.md) | [08-正式局设计系统](./08-正式局设计系统.md)  
> **维护说明**：本文为模拟战 / Admin / 全物品池的视觉与组件权威说明（`--sb-*` / `--adm-*`）。玩法规则见 `01`～`06`。
...
- **战斗属性**：HP / 耐力 / 耐力恢复/秒 / HP恢复/秒 / 负重上限；**可折叠**，五字段全为 0 时默认折叠
- **主动动作**：主动动作（有/无）+ 条件展开；二级分区 **主动消耗** → **主动目标** → **主动效果**（效果库引用 EffectPicker，先建效果再挂）
- **被动加成**：被动总开关 + 被动目标 + 效果库引用（按 allowPassive 过滤）

### 词条表单分区结构

- **基本信息**：ID / 名称 / 分类 / **描述**（原效果文案，不参与结算） / 价值 / 槽耗 / 可重复
- **前置条件**：前置词条 / 池前置
- **主动目标覆写**：覆写模式（不修改/修改）+ targetingModifier 字段
- **主动效果**：效果库引用（修饰宿主主动，非独立开火）
- **被动加成**：被动总开关 + 被动目标 + 效果库引用

### 效果库 Tab

独立 CRUD：配方名、allowActive/allowPassive、类型/属性/运算/默认参数/持续/跳伤。仍被实体或词条引用时禁止删除；大改保存可提示引用件数。
...
启动页「全物品池」：浏览全部实体 / 词条 / **效果库**模板，**无写操作**。视觉与交互色继承 `#adm-page`（`#ip-page` 镜像同一套 token）。

### 布局

- 顶栏：返回 · 标题 · 下划线 Tab（实体 / 词条 / **效果**）
- 左栏 300px：分类 Chip（实体/词条与制作物品一致；效果 Tab 无分类 Chip）→ Ctrl+K 搜索 → 列表
- 右栏：只读表单分区；实体/词条字段对齐 Admin；效果详情展示配方参数与可引用通道

### 详情约定

- 固定词条 / 预装动态词条 / 前置词条：**一词条一行**（名称 · 类别 · **描述**）；**不展示池前置**
- 实体/词条主动与被动区展示**已解析**效果文案（与战斗一致）
- `isActive` / 被动总开关 / targeting 覆写模式：同 adm 条件显隐
...
- 玩法：`design/01`～`06`
- 正式局壳层 UI：`design/08-正式局设计系统.md`（色值对齐本文 `--sb-*`）
- 目录：[README.md](./README.md)

## 修订记录

- 2026-07-20：模拟对战设计系统初稿
- 2026-08-14：补充 `--sb-card-border`（卡片外框深于普通分割线）
- 2026-08-17：纳入编号系列，更名为 `07-模拟对战设计系统.md`（曾用名 `DESIGN.md`）
