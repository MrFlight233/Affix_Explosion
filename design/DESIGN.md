...
- **战斗属性**：HP / 耐力 / 耐力恢复/秒 / HP恢复/秒 / 负重上限；**可折叠**，五字段全为 0 时默认折叠（见 [doc/2026-08-06-战斗属性区可折叠.md](../doc/2026-08-06-战斗属性区可折叠.md)）
- **主动动作**：主动动作（有/无）+ 条件展开；二级分区 **主动消耗**（耐力消耗、触发耗时）→ **主动目标**（排序/过滤/数量）→ **主动效果**（onHitEffects）
- **被动加成**：被动加成模式 + 被动目标 + 被动效果

### 词条表单分区结构

- **基本信息**：ID / 名称 / 分类 / 效果描述 / 价值 / 槽耗 / 可重复
- **前置条件**：前置词条 / 池前置
- **主动目标覆写**：覆写模式（不修改/修改）+ targetingModifier 字段
- **主动效果**：onHitEffects（修饰宿主主动，非独立开火）
- **被动加成**：被动加成模式 + 被动目标 + 被动效果
...
- 固定词条 / 预装动态词条 / 前置词条：**一词条一行**（名称 · 类别 · 效果）；**不展示池前置**（`poolPrerequisite` 仅 Admin 可编辑）
